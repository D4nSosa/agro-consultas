  ...
  render() {
    ...
    container.innerHTML = `
      <div class="timeline-header" style="margin-bottom: 15px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px;">
        <div>
          <h4 style="margin: 0; color: var(--verde-principal);">📅 Serie Temporal de Adquisiciones Sentinel-2</h4>
        ...
        <div style="display: flex; gap: 15px; align-items: center;">
          <label style="font-size: 0.85rem; font-weight: bold;">
            🟢 Fecha A (Base):
            <select id="select-date-a" style="padding: 5px 8px; border-radius: 6px; border: 1px solid var(--borde-suave); margin-left: 5px;">
          ...
          <label style="font-size: 0.85rem; font-weight: bold;">
            🔵 Fecha B (Comparación):
            <select id="select-date-b" style="padding: 5px 8px; border-radius: 6px; border: 1px solid var(--borde-suave); margin-left: 5px;">
      ...
      <div class="timeline-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px;">
        ${this.timelineData.map(item => {
          ...
          if (isA) {
            ...
            badge = '<span class="badge" style="background:#27ae60; color:#fff; font-size:0.7rem; padding:2px 6px; border-radius:4px; margin-left:5px;">Fecha A</span>';
          } else if (isB) {
            activeClass = 'border-left: 4px solid #2980b9; background: rgba(41, 128, 185, 0.08);';
            badge = '<span class="badge" style="background:#2980b9; color:#fff; font-size:0.7rem; padding:2px 6px; border-radius:4px; margin-left:5px;">Fecha B</span>';
          }

          return `
            <div class="card timeline-card" style="padding: 12px; border-radius: 8px; ${activeClass}">
              ...
              <div style="font-size: 0.75rem; color: var(--texto-secundario); margin-top: 4px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">🆔 ${item.product}</div>
            </div>
          `;
        }).join('')}
      </div>
